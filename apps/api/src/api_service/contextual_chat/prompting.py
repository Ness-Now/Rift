...
def load_prompt_asset() -> PromptAsset:
    ...
    if not metadata_path.exists() or not system_path.exists():
        raise ContextualChatValidationError("Contextual chat prompt assets are missing.")
    ...
    if system_prompt == "":
        raise ContextualChatValidationError("Contextual chat system prompt is empty.")
