...
- The web app now uses disciplined local/system font stacks behind the existing `--font-sans` and `--font-display` variables instead of `next/font/google`, removing build-time outbound font fetching.
...
- `npm run build:web` no longer depends on outbound Google Fonts access at build time.
